...
fn managed_recovery_and_removal() {
    ...
    // Branch preserved.
    assert!(is_hex40(
        &resolve_ref(td.path(), "agent-hub/chat/chat9").unwrap()
    ));
}
...
fn provision_rejects_existing_managed_branch() {
    let td = init_repo();
    let base = resolve_ref(td.path(), "main").unwrap();
    let branch = "agent-hub/chat/already";
...
fn rollback_deletes_only_at_expected_tip() {
    let td = init_repo();
    let base = resolve_ref(td.path(), "main").unwrap();
    let branch = "agent-hub/chat/rollback";
    git(td.path(), &["branch", branch]);
...
fn rollback_refuses_a_moved_branch() {
    let td = init_repo();
    let base = resolve_ref(td.path(), "main").unwrap();
    let branch = "agent-hub/chat/moved";
    git(td.path(), &["branch", branch]);
...
fn recovery_repairs_stale_worktree_metadata_without_losing_files() {
    ...
    fs::write(admin.join("gitdir"), "/tmp/old-agent-hub-worktree/.git\n").unwrap();
