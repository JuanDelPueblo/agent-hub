  ...
  template: `
    @if (project(); as current) {
      <section class="project-page">
        <div class="hero"><div><p class="eyebrow">Project</p><h1>{{ current.name }}</h1><code>{{ current.path }}</code></div><div class="hero-actions"><button mat-flat-button color="primary" type="button" (click)="newChat(current.id)"><mat-icon>add_comment</mat-icon> New chat</button><button mat-icon-button [matMenuTriggerFor]="projectActions" aria-label="Project actions"><mat-icon>more_vert</mat-icon></button><mat-menu #projectActions="matMenu"><button mat-menu-item type="button" (click)="edit(current)"><mat-icon>edit</mat-icon><span>Edit project</span></button><button mat-menu-item type="button" (click)="remove(current)"><mat-icon color="warn">delete</mat-icon><span>Delete project</span></button></mat-menu></div></div>
        ...
        } @else { <mat-card class="empty-chat"><mat-icon>forum</mat-icon><h2>No chats in this project yet</h2><p>Start a new chat with an ACP agent.</p><button mat-flat-button color="primary" type="button" (click)="newChat(current.id)"><mat-icon>add_comment</mat-icon> New chat</button></mat-card> }
      </section>
    } @else { <section class="missing"><mat-icon>folder_off</mat-icon><h1>Project not found</h1><p>This project may have been deleted or the URL is malformed.</p><a mat-flat-button color="primary" routerLink="/">Back to projects</a></section> }
  `,
  styles: `
    :host { display: block; height: 100%; overflow: auto; } .project-page { max-width: 1120px; margin: 0 auto; padding: 40px 32px 72px; }
    .hero { display: flex; align-items: center; justify-content: space-between; gap: 24px; padding: 32px; margin-bottom: 36px; border-radius: 28px; background: linear-gradient(120deg, var(--mat-sys-surface-container-low), var(--mat-sys-primary-container)); } .eyebrow { margin: 0 0 6px; color: var(--mat-sys-primary); font-size: .8rem; font-weight: 700; letter-spacing: .08em; text-transform: uppercase; } h1 { margin: 0 0 8px; font-size: clamp(1.8rem, 4vw, 3rem); } code { overflow-wrap: anywhere; color: var(--mat-sys-on-surface-variant); }
    .hero-actions { display: flex; align-items: center; gap: 8px; } .section-title { display: flex; align-items: baseline; justify-content: space-between; gap: 16px; margin-bottom: 16px; } .section-title h2 { margin: 0; font-size: 1.5rem; } .section-title span { color: var(--mat-sys-on-surface-variant); }
    .chat-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 16px; } .chat-card { transition: transform 160ms ease, box-shadow 160ms ease; } .chat-card:hover { transform: translateY(-2px); box-shadow: var(--mat-sys-level2); } .chat-card a { display: block; padding: 20px; color: inherit; text-decoration: none; } .chat-top { display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-bottom: 26px; } .agent-badge { padding: 5px 10px; border-radius: 999px; background: var(--mat-sys-secondary-container); color: var(--mat-sys-on-secondary-container); font-size: .8rem; } .process-state { color: var(--mat-sys-on-surface-variant); font-size: .75rem; } .process-state.running { color: var(--hub-status-running); } .process-state.dead { color: var(--hub-status-dead); }
