...
@Component({
  ...
  template: `
    <mat-card class="permission" [class.responded]="permission.responded">
      ...
      @else { <mat-card-actions align="end"><button mat-stroked-button type="button" (click)="respond(false)" [disabled]="responding()">Deny</button><button mat-flat-button color="primary" type="button" (click)="respond(true)" [disabled]="responding()">@if (responding()) { <mat-spinner diameter="18" /> } @else { Allow }</button></mat-card-actions> }
    </mat-card>
  `,
  styles: `
    :host { display: block; margin: 12px 0; } .permission { border: 1px solid color-mix(in srgb, var(--mat-sys-error) 50%, transparent); background: var(--mat-sys-error-container); } .permission.responded { border-color: var(--mat-sys-outline-variant); background: var(--mat-sys-surface-container-low); opacity: .88; } mat-card-header { align-items: center; } mat-card-avatar { color: var(--mat-sys-error); } mat-card-content { padding-top: 12px; } pre { margin: 0; padding: 12px; overflow: auto; border-radius: 10px; background: color-mix(in srgb, var(--mat-sys-surface-container-lowest) 80%, transparent); white-space: pre-wrap; font: inherit; } .decision { display: flex; align-items: center; gap: 6px; padding: 0 16px 14px; color: var(--mat-sys-on-surface-variant); font-size: .85rem; }
  `,
})
